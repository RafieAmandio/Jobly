...
class GlintsProvider(DirectScrapeProvider):
    name = "glints"
    ...
    async def fetch_jobs(self, filters: JobFilters) -> list[JobResult]:
        ...
        try:
            response = await client.get(
                f"{self.base_url}/id/opportunities/jobs/explore",
                params={
                    "keyword": query,
                    "country": "ID",
            ...
            logger.exception("Glints: failed to fetch page")
            return []
    ...
    def _parse_html(self, html: str) -> list[JobResult]:
        ...
        job_cards = soup.select("[data-testid='job-card'], .JobCard, .job-card, article")
