...
async def _extract_cv_text_from_document(message: Message, lang: str) -> str | None:
    ...
    await message.answer(
        "Please upload a PDF file." if lang == "en" else "Mohon upload file PDF."
    )
    return None
...
async def on_cv_upload(
    ...
    cv_text = await _extract_cv_text_from_document(message, lang)
    if cv_text is None:
        return
...
async def on_cv_text_upload(
    ...
    cv_text = message.text.strip() if message.text else ""
    await _store_current_cv(session, db_user, cv_text)
    await state.clear()
    await message.answer(t("cv_uploaded", lang))
